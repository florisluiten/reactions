...
CREATE TABLE IF NOT EXISTS `reactions`.`reactions` ( `reactionID` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, `parentID` BIGINT UNSIGNED NULL DEFAULT NULL, `articleID` BIGINT UNSIGNED NOT NULL, `score` INT NULL DEFAULT 0, `userID` BIGINT UNSIGNED NOT NULL, `publishDate` DATETIME NOT NULL, `content` TEXT NOT NULL, PRIMARY KEY (`reactionID`), UNIQUE INDEX `reactionID_UNIQUE` (`reactionID` ASC), INDEX `fk_reactions_1_idx` (`parentID` ASC), INDEX `fk_reactions_3_idx` (`userID` ASC), INDEX `fk_reactions_2_idx` (`articleID` ASC), CONSTRAINT `fk_reactions_1` FOREIGN KEY (`parentID`) REFERENCES `reactions`.`reactions` (`reactionID`) ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT `fk_reactions_2` FOREIGN KEY (`articleID`) REFERENCES `reactions`.`articles` (`articleID`) ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT `fk_reactions_3` FOREIGN KEY (`userID`) REFERENCES `reactions`.`users` (`userID`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE = InnoDB;
